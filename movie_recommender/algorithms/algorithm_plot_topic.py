...
# def main():
#     print(get_plot_based_recommendation(34,5))
# if __name__ == "__main__":
#     main()
# import os
# import json

def count_movie_json(data_dir):
    total       = 0
    has_meta    = 0
    has_title   = 0
    has_plot    = 0
    ...
    for fn in os.listdir(data_dir):
        ...
        if md:
            has_meta += 1
        # title non-empty?
        if md.get("title", "").strip():
            has_title += 1
        # plotSummary non-empty?
        if md.get("plotSummary", "").strip():
            has_plot += 1
        # movieId present?
        if md.get("movieId") is not None:
            has_id += 1

    print(f"Total JSON files:          {total}")
    print(f"With any movielens block:  {has_meta}")
    print(f"With non‐empty titles:     {has_title}")
    print(f"With non‐empty plots:      {has_plot}")
    print(f"With movieId present:      {has_id}")
    print()
    print("→ The number you actually load is the count where all four are true.")
    print(f"   Expect ~{min(has_meta,has_title,has_plot,has_id)} loaded movies.")

if __name__ == "__main__":
    this_dir = os.path.dirname(__file__)
    data_dir  = os.path.abspath(os.path.join(this_dir,
                         "..", "datasets", "extracted_content_ml-latest"))
    count_movie_json(data_dir)
